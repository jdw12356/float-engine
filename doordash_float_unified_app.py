# Doordash Float Arbitrage Streamlit App with Alpaca Integration
# ---------------------------------------------------------------
# Instructions:
# 1. Run: pip install streamlit yfinance pandas numpy matplotlib seaborn alpaca-trade-api
# 2. Launch: streamlit run doordash_float_unified_app.py
...
# ---- Alpaca Paper Trading Keys (Replace for Live) ----
...
def simulate_order_type():
    return random.choices(list(SECTOR_ETF_MAP.keys()), weights=[0.5, 0.3, 0.2])[0]
...
st.title("📈 Doordash Float Arbitrage Engine + Alpaca Trading")
...
if run_sim:
    ...
    st.line_chart(df.set_index("Date")[["1M Yld", "3M Yld", "1Y Yld"]])
